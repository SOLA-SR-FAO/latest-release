...
CREATE TABLE notation (
    ...
    rrr_id character varying(40),
    transaction_id character varying(40) NOT NULL,
    reference_nr character varying(100) NOT NULL,
...
CREATE TABLE notation_historic (
    ...
    rrr_id character varying(40),
    transaction_id character varying(40),
    reference_nr character varying(100),
    notation_text character varying(1000),
    notation_date timestamp without time zone,
    status_code character varying(20),
    rowidentifier character varying(40),
...
CREATE TABLE rrr (
    id character varying(40) NOT NULL,
    ba_unit_id character varying(40) NOT NULL,
    nr character varying(100) NOT NULL,
    type_code character varying(20) NOT NULL,
    status_code character varying(20) DEFAULT 'pending'::character varying NOT NULL,
...
CREATE TABLE source (
    ...
    la_nr character varying(100) NOT NULL,
    reference_nr character varying(255),
...
-- DROP INDEX cadastre.sr_work_unit_index_on_rowidentifier;
...
-- Table: cadastre.sr_work_unit_historic

-- DROP TABLE cadastre.sr_work_unit_historic;

CREATE TABLE cadastre.sr_work_unit_historic
(
  id character varying(150),
  "name" character varying(150),
  parcel_estimated numeric,
  public_display_start_date date,
  parcel_recorded numeric,
  claims_recorded numeric,
  claims_scanned numeric,
  demarcation_scanned numeric,
  certificates_distributed numeric,
  rowidentifier character varying(40),
  rowversion integer,
  change_action character(1),
  change_user character varying(50),
...
CREATE INDEX sr_work_unit_historic_index_on_rowidentifier
  ON cadastre.sr_work_unit_historic
  USING btree
  (rowidentifier);


DROP TRIGGER IF EXISTS __track_history ON cadastre.sr_work_unit CASCADE;
CREATE TRIGGER __track_history AFTER UPDATE OR DELETE
   ON cadastre.sr_work_unit FOR EACH ROW
...
CREATE TABLE administrative.dispute_comments(
    id varchar(50) NOT NULL,
    dispute_nr varchar(100) NOT NULL,
...
CREATE INDEX dispute_comments_other_authorities_code_fk89_ind ON administrative.dispute_comments (other_authorities_code);
    ...
    ALTER TABLE  administrative.ba_unit ADD COLUMN ground_rent integer DEFAULT 0;
...
CREATE OR REPLACE VIEW administrative.systematic_registration_listing AS 
 SELECT DISTINCT co.id, 
  ...
  WHERE (co.name_firstpart::text || co.name_lastpart::text) = (ap.name_firstpart::text || ap.name_lastpart::text) 
	...
	AND  public.st_intersects(public.st_pointonsurface(co.geom_polygon), sg.geom)
  UNION 
         SELECT DISTINCT co.id, 
						 co.name_firstpart, 
						 co.name_lastpart, 
						 round(sa.size, 0) AS size, 
						 co.land_use_code, ''::text AS ba_unit_id, 
						 sg.name::text AS name, ''::text AS property_location
           FROM cadastre.cadastre_object co, 
				cadastre.spatial_value_area sa, 
				cadastre.spatial_unit_group sg, 
				application.application_property ap
          WHERE co.status_code::text = 'current'::text 
		  AND sa.spatial_unit_id::text = co.id::text 
		  AND sa.type_code::text = 'officialArea'::text 
		  AND public.st_intersects(public.st_pointonsurface(co.geom_polygon), sg.geom) 
		  AND sg.hierarchy_level = 4 
		  AND NOT (co.name_firstpart::text || co.name_lastpart::text IN ( SELECT ap.name_firstpart::text || ap.name_lastpart::text
...
CREATE OR REPLACE VIEW administrative.sys_reg_state_land AS 
 ...
 co.name_firstpart, co.name_lastpart, public.get_translation(lu.display_value, NULL::character varying) AS land_use_code, 
...
----------------   cadastre object view  read only-------------------------  

-- DROP GEOMETRY columns registration from geometry_columns table
DELETE FROM public.geometry_columns WHERE f_table_schema = 'cadastre' AND f_table_name='current_parcels' AND f_geometry_column = 'geom_polygon';
DELETE FROM public.geometry_columns WHERE f_table_schema = 'cadastre' AND f_table_name='pending_parcels' AND f_geometry_column = 'geom_polygon';
DELETE FROM public.geometry_columns WHERE f_table_schema = 'cadastre' AND f_table_name='historic_parcels' AND f_geometry_column = 'geom_polygon';
DELETE FROM public.geometry_columns WHERE f_table_schema = 'cadastre' AND f_table_name='current_pending' AND f_geometry_column = 'geom_polygon';
...
SELECT '', 'cadastre', 'current_parcels', 'geom_polygon', public.ST_CoordDim(geom_polygon), public.ST_SRID(geom_polygon), public.GeometryType(geom_polygon)
FROM cadastre.current_parcels LIMIT 1;

INSERT INTO public.geometry_columns(f_table_catalog, f_table_schema, f_table_name, f_geometry_column, coord_dimension, srid, "type")
SELECT '', 'cadastre', 'pending_parcels', 'geom_polygon', public.ST_CoordDim(geom_polygon), public.ST_SRID(geom_polygon), public.GeometryType(geom_polygon)
FROM cadastre.pending_parcels LIMIT 1;

INSERT INTO public.geometry_columns(f_table_catalog, f_table_schema, f_table_name, f_geometry_column, coord_dimension, srid, "type")
SELECT '', 'cadastre', 'historic_parcels', 'geom_polygon', public.ST_CoordDim(geom_polygon), public.ST_SRID(geom_polygon), public.GeometryType(geom_polygon)
FROM cadastre.historic_parcels LIMIT 1;

INSERT INTO public.geometry_columns(f_table_catalog, f_table_schema, f_table_name, f_geometry_column, coord_dimension, srid, "type")
SELECT '', 'cadastre', 'current_pending_parcels', 'geom_polygon', public.ST_CoordDim(geom_polygon), public.ST_SRID(geom_polygon), public.GeometryType(geom_polygon)
