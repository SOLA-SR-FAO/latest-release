...
INSERT INTO br (id, display_name, technical_type_code, feedback, description, technical_description) VALUES ('application-on-approve-check-systematic-reg-no-objection', 'application-on-approve-check-systematic-reg-no-objection', 'sql', 'There must be no objection against the systematic registration::::Не должно быть каких-либо обжалований по системной регистрации.::::يجب ان لا يوجد اعتراض على التسجيل المنتظم::::Il ne doit pas y avoir d''objection à l''enregistrement systématique.::::::::::::Não deve haver nenhuma objeção no registro regular::::::::针对系统登记不得有异议。', NULL, 'Checks the absence of objections for systematic registration service related to the application');
...
INSERT INTO br (id, display_name, technical_type_code, feedback, description, technical_description) VALUES ('bulk-spatial-geom-overlaps-with-loading-geom', 'bulk-spatial-geom-overlaps-with-loading-geom', 'sql', 'New cadastre objects must not overlap with other new cadastre objects. ::::Новые кадастровые объекты не должны пересекаться с другими новыми объектами.::::الكينونات المساحية الجديدة لا يجب ان تتداخل مع كينونات مساحة  جديدة أخرى::::::::::::::::Novos objetos de cadastro não devem se sobrepor com outros novos objetos de cadastro.::::::::新的地籍目标不能与其他新的地籍目标相交叠。 ', NULL, '#{id}(transaction_id) is requested');
...
INSERT INTO br (id, display_name, technical_type_code, feedback, description, technical_description) VALUES ('ba_unit-has-caveat', 'ba_unit-has-caveat', 'sql', 'Caveat should not prevent registration proceeding.::::Арест не должен ограничивать дальнейшую регистрацию.::::القيود لا يجب ان تعطل عملية التسجيل::::Un caveat ne doit pas empêcher de procéder à l''enregistrement.::::::::::::Embargo não deve impedir a continuidade da inscrição.::::::::附加权利不应阻止登记的进行。', NULL, '#{id}(administrative.rrr.id) is requested.');
INSERT INTO br (id, display_name, technical_type_code, feedback, description, technical_description) VALUES ('service-has-person-verification', 'service-has-person-verification', 'sql', 'Within the application for the service a personal identification verification should be attached.::::В заявлении должен присутствовать документ удостоверяющий личность.::::ضمن الطلب ولاجل الخدمة يجب  ارفاق وثيقة تعريف بالشخصية::::Pour la demande d''un service, la vérification de l''identification personnelle doit être attachée.::::::::::::No pedido, para a realização do serviço, uma identificação pessoal deve ser anexada para verificação.::::::::申请内要附上个人身份证明。', NULL, '#{id}(application.service.id) is requested');
...
INSERT INTO br (id, display_name, technical_type_code, feedback, description, technical_description) VALUES ('public-display-check-baunit-has-co', 'public-display-check-baunit-has-co', 'sql', 'All property must have an associated cadastre object.::::Все объекты недвижимости должны иметь соответствующие кадастровые объекты.::::جميع الملكيات يجب ان ترتبط بكائن مساحي::::::::::::::::Toda propriedade deve ter um objeto de cadastro associado.::::::::所有的财产都必须有一个相关的地籍目标。', NULL, '#{lastPart}(name_lastpart) is requested');
...
INSERT INTO br (id, display_name, technical_type_code, feedback, description, technical_description) VALUES ('document-supporting-rrr-is-current', 'document-supporting-rrr-is-current', 'sql', 'Documents supporting rights (or restrictions) must have current status::::Документы использующиеся для регистрации прав или ограничений должны иметь текущий (активный) статус.::::الوثائق التي تدعم الحقوق او القيود يجب ان يكون لها الحالة الحالية::::Les documents justificatifs de droits (ou restrictions) doivent avoir le statut courant.::::::::::::Documentos que servem de suporte a direitos (ou restrições) devem ter o estado atual::::::::支持（或限制）权利的文件必须为目前状态。', NULL, '#{id}(application.service.id)');
...
insert into system.br_validation(id,br_id,target_code,target_service_moment,target_request_type_code,severity_code,order_of_execution) VALUES(public.uuid_generate_v1(),'check-registered-SR-nr-is-unique','service','complete','registerSRCofO','critical',1);

--validation for approve
insert into system.br_validation(id,br_id,target_code,target_service_moment,target_request_type_code,severity_code,order_of_execution) VALUES(public.uuid_generate_v1(),'check-registered-SR-nr-is-unique','service','lodge','registerSRCofO','critical',2);
...
-------- spatial-unit-group-inside-other-spatial-unit-group -----------
DELETE FROM system.br_validation where br_id='spatial-unit-group-inside-other-spatial-unit-group'; 
DELETE FROM system.br_definition where br_id='spatial-unit-group-inside-other-spatial-unit-group'; 
DELETE FROM system.br where id='spatial-unit-group-inside-other-spatial-unit-group'; 
